...
**Lifetime quota** (`schema/027_free_tier_share_cap.sql:enforce_share_cap` + `packages/app/src/lib/quota.ts`). Free tier caps at 2 lifetime tracked links (rows in `document_shares`) — revoked and expired links count, so a user can't rotate slots by deleting and re-creating. Documents themselves are uncapped. The Postgres trigger (`BEFORE INSERT` on `document_shares`, Pro owners exempt, per-owner advisory lock so concurrent inserts can't race past the cap) raises before the 3rd link; the UI counter on `/settings`, `/upgrade`, and the doc share panel reads `readQuota()`, which counts `document_shares WHERE owner_id = ...` so they agree. The `createShareAction` pre-check routes Free users at the cap to `/upgrade?reason=share_quota`, and a backstop catch maps the trigger's raised error to the same redirect. (The old per-document cap `003_triggers.sql:enforce_doc_cap` is dropped by `027`.)
