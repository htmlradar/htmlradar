-- 013_internal_viewer_no_notify.sql
-- ------------------------------------------------------------
-- notify_on_first_open: skip when the viewer is marked internal.
--
...
--
-- The `share.first_view` app_events row STILL fires, even for internal
-- views — telemetry should capture every read for observability. Only
-- the user-facing email is suppressed.
--
...
begin
  ...
  -- INTERNAL VIEWER GUARD: this is the only behavioral change vs 010.
  -- Owner-self previews, staff QA, and `@htmlradar.com` test addresses
  -- should never produce a "someone opened your deck" email. The viewer
  -- is the owner themselves (or someone they explicitly QA-flagged) and
  -- the noise was confusing in real prod — "verify+post-declutter@
  -- htmlradar.com opened ..." emails to the owner reading their own
  -- deck. Telemetry above still fires; only the email is suppressed.
  ...
  -- Body builder identical to 010. Keep in lockstep with that file.
...
$html$,
  ...
  begin
    ...
    insert into notifications_log (session_id, email_to, status, request_id)
    values (new.id, v_owner.email, 'sent', v_request_id);
  exception when others then
    insert into notifications_log (session_id, email_to, status, error_message)
