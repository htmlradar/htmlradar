...
- **`/docs/api`** — the HTTP API reference. `MESSAGE-ARCHITECTURE-2026-09-04.md` named the API as one of the four doors into HTMLRadar but found no page documented it; this closes that gap. Every `/api/v1` route a customer can call — `GET /me`, `POST` and `GET /shares`, `GET /shares/{id}/activity`, `POST /shares/{id}/revoke`, `GET /documents`, `POST /documents/{id}/replace` — with a wrapping curl example and the exact JSON response, read from the route code rather than guessed. `packages/app/src/middleware.ts` and `src/app/robots.ts` gained a narrow exception so this one page is public while `/docs/[id]`, the private per-document dashboard under the same `/docs` prefix, stays gated. Linked from the site footer, `/mcp`'s related-links section, and the README's API-key instructions.
